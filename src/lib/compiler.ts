...
export function underline_code(
    ...
    const view_start_line =
        lineno - count_ocurrences(source.slice(view_begin, pos), "\n")
    const prefix = Array.from(
        new Array(view.length),
        (_, i) => `${view_start_line + i} |`,
    )
...
export class Compiler {
    ...
    compile(
        ...
        try {
            ...
            const ast = stream.provide({ post: this.post }, () => this.parse(stream))
